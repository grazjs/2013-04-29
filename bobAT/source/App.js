enyo.g11n.setLocale({ sourceLocale: "de_de"});
...
enyo.kind({
	...
	components:[
		...
		{kind: "enyo.Scroller", fit: true, classes: "bobat", components: [
			...
			},
			{
				...
				components: [
					{kind: "onyx.InputDecorator", style: "background-color: #ffffff", components: [
						{ name: "phoneNmbr", kind: "onyx.Input", value: '', placeholder: $L("Telefonnummer"), onchange: "numberChanged"}
					]},
					{kind: "onyx.InputDecorator", style: "background-color: #ffffff", components: [
						{ name: "pwd", kind: "onyx.Input", value: '', placeholder: $L("Passwort"), type: 'password', onkeydown: "searchOnEnter"/*, onchange: "pwdChanged"*/}
					]},
					{tag: "br"},
					{kind: "onyx.Button", content: $L("Schließen"), ontap: "closeSettingsPopup"},
					{kind: "onyx.Button", content: $L("Speichern"), ontap: "saveAndCloseSettingsPopup"}
				]
			},
			...
			{name: "error", allowHtml: true, content: "", style: "padding: 8px;color: red;"},
			{name: "dateExample", allowHtml: true, content: "", style: "padding: 8px;color: red;"}
		]}
	],
	...
	load: function(inSender, inEvent) {
		...
		// console.log(enyo.g11n.currentLocale().getLocale());

		// var fmt = new enyo.g11n.DateFmt({
		// 	date: "short",
		// 	time: "short",
		// 	locale: new enyo.g11n.Locale("de_at")
		// });

		//enyo.g11n.setLocale({uiLocale: "de_at"});

		//this.$.dateExample.setContent("Current date  = " + fmt.format(new Date()));
	},
	...
	dataError: function (inSender, inError) {
		this.$.clientInfoContainer.setShowing(false);
		this.$.costManagerContainer.setShowing(false);
